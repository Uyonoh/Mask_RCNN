...
class ProposalLayer(KL.Layer):
    ...
    def call(self, inputs):
        ...
        if not context.executing_eagerly():
            # Infer the static output shape if eager execution disabled
...
class DataGenerator(KU.Sequence):
    ...
    def __next__(self):
        inputs, outputs = self.__getitem__(self.next_idx)
        self.next_idx += 1
...
class MaskRCNN(object):
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
              augmentation=None, custom_callbacks=None, no_augmentation_sources=None, save_best=False):
        """Train the model.
        ...
        save_best: Optional. Determines if the model only saves at the current 
            best validator
        """
        assert self.mode == "training", "Create model in training mode."

        # Pre-defined layer regular expressions
        print("Generating regex for layer ...")
        layer_regex = {
        ...
        callbacks = [
            ...
            keras.callbacks.ModelCheckpoint(self.checkpoint_path,
                                            verbose=0, save_best_only=save_best, save_weights_only=True),
        ]

        # Add custom callbacks to the list
        if custom_callbacks:
            print("Adding custom callbacks ...")
